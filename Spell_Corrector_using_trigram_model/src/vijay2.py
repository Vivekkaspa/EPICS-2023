...
def is_corrected(sentence):
    ...
    corrected_text = corrected_sentence['result']
    return sentence == corrected_text
...
# Define a function to copy selected text to the clipboard
def copy_text():
    text = text_widget.get("sel.first", "sel.last")
    if text:
        root.clipboard_clear()
        root.clipboard_append(text)
        tkmb.showinfo("Copy Text", "Text copied to clipboard.")
    else:
        tkmb.showwarning("Copy Text", "No text selected.")
